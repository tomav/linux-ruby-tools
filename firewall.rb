...
ipt_policy.each do |f|
  system("sudo #{IPTABLES} -P #{f} DROP")
end
puts "[ OK ]"

print "> Setting Up NAT                               "
system("sudo #{IPTABLES} -F -t nat")
system("sudo #{IPTABLES} -P FORWARD DROP")
system("sudo #{IPTABLES} -A FORWARD -i #{IFACE_NAME} -j ACCEPT")
system("sudo #{IPTABLES} -A INPUT -i #{IFACE_NAME} -j ACCEPT")
system("sudo #{IPTABLES} -A OUTPUT -o #{IFACE_NAME} -j ACCEPT")
puts "[ OK ]"

print "> Allow ICMP & loopback packets                "
system("sudo #{IPTABLES} -A INPUT -p icmp -j ACCEPT")
...
ipt_input_ports_udp.each do |f|
  system("sudo #{IPTABLES} -A INPUT -p udp --dport #{f} -j ACCEPT")
end
puts "[ OK ]"
